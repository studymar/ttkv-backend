        ...
        <?php $form = ActiveForm::begin(FormHelper::getConfigArray());?>
            ...
            <div class="d-flex justify-content-between p-1">
                <div class="form-group">
                    <a href="<?= Url::toRoute(['vereinsmeldung/vereinskontakte','p'=>$vereinsmeldungKontakte->vereinsmeldung_id]) ?>" class="btn btn-outline-secondary">Abbrechen</a>
                    <?= Html::submitButton('Speichern', ['class' => 'btn btn-primary']) ?>
                </div>
                <div>
                    <?php if($person->isDeletable()){ ?>
                    <a href="<?= Url::toRoute(['vereinsmeldung/delete-person','p'=>$person->id]) ?>" class="btn btn-danger">Löschen</a>
                    <?php } else { ?>
